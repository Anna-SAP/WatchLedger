...
- 应用只监听本机回环地址 `127.0.0.1:17643` 和 `[::1]:17643`，每次数据请求需要配对码；拒绝普通外部网站来源与非本机 Host。系统未启用 IPv6 时仍可使用 IPv4。
...
安装桥接后，可运行 `node tests/launcher-smoke.cjs` 验证真实电源按钮、本地启动、自动配对、绿/灰图标、回顾页，以及关闭浏览器后服务仍然运行。该测试使用隔离浏览器配置，连接本机默认服务，不创建或删除观看记录，并保留启动的服务供用户使用。可通过 `BROWSER_EXECUTABLE` 指定 Opera，通过 `EXPECT_ENDPOINT` 验证选中的回环地址。Neo 与 Firefox 还需要各自在实际浏览器中验证。

### Opera 本机连接兼容

在本机 Opera 135 / Chromium 151 的独立配置中，曾复现 IPv4 回环地址返回 `net::ERR_ADDRESS_INVALID`，同时 IPv6 回环地址可正常访问；该现象不代表所有 Opera 环境都会失败。0.1.2 同时提供 IPv4 / IPv6 本机服务；扩展遇到网络错误时尝试另一个回环地址并记住可用地址，“打开回顾页面”也使用已验证的连接地址。认证错误仍显示配对失败，不会改用其他地址绕过认证。

从旧版升级后，需要重启正在运行的本地服务才能启用 IPv6 监听，再在浏览器中重新加载原扩展。无需卸载扩展、清除记录或关闭浏览器安全设置。外部网站能否访问互联网 IPv6 与本机 `::1` 回环是否可用是两回事。
