# 视听档案 WatchLedger 0.1.0

可加载的本地原型：Neo / Chromium 扩展 + Firefox 测试扩展 + Python/SQLite 服务 + 中文回顾页面。

## 首次使用

1. 将整个 WatchLedger 文件夹保留在固定位置，双击 **Start.cmd**。当前电脑可使用 Codex 已有 Python；其他电脑需要 Python 3.10+。首次启动会打开回顾页面。
2. Neo 打开扩展管理页，开启 Developer mode，选择 **Load unpacked**，选择本文件夹中的 **neo** 子文件夹（里面有 manifest.json）。
3. 回顾页面展开“连接与配对设置”，点击“复制配对码”。在扩展弹窗粘贴，点击“保存并连接”。弹窗应显示“已连接本机”。
4. 刷新已打开的节目页面。正常播放后约 30 秒，扩展同步数据，页面再自动刷新；也可以点击刷新。
5. Firefox 打开 `about:debugging#/runtime/this-firefox` → 临时载入附加组件 → 选择 **firefox/manifest.json**。在扩展权限中允许访问观看网站，粘贴同一配对码。
...
Neo 加载和配对完成后无需逐条记录。保持 Start.cmd 服务窗口运行；窗口内 Ctrl+C 停止服务。服务停止时扩展仍保存待同步队列，下次启动再同步。
