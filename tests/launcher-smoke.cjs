...
(async()=>{
 ...
 console.log(JSON.stringify({ok:true,extensionId,output,checks:['real native host launch','automatic pairing','green connected icon','gray failed-auth icon','popup reopen','unsaved pairing draft preserved','reconnect existing service','service survives browser exit','no page errors']},null,2));
